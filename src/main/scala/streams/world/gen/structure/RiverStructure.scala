...
class RiverStructure(
  ...
  def generate()(implicit worldAccess: IBlockAccess, random: Random) {
    random.shuffle(CardinalDirections).foreach {
      case North =>
        for (x <- slide(xMin, xMax, XPlanSize))
          if (createMouth(South, XPlanSize, ZPlanSize, x, zMin + ShorePadding))
            return
      case South =>
        for (x <- slide(xMin, xMax, XPlanSize))
          if (
            createMouth(
              ...
              ZPlanSize,
              x,
              zMax - ZPlanSize - ShorePadding + 1
            )
          ) return
      case West =>
        for (z <- slide(zMin, zMax, ZPlanSize))
          if (createMouth(East, ZPlanSize, XPlanSize, xMin + ShorePadding, z))
            return
      case East =>
        for (z <- slide(zMin, zMax, ZPlanSize))
          if (
  ...
  private def createMouth(
      downstreamOrientation: Direction,
      xSize: Int,
      zSize: Int,
      xMin: Int,
      zMin: Int
  )(implicit worldAccess: IBlockAccess, random: Random) = {
    val mouth = new RiverMouthComponent(
      this,
      sizedBox(xMin, zMin, xSize, zSize),
  ...
  def carveValleys(implicit blockArray: ChunkBlockArrayAccess, random: Random) {
    for (x <- blockArray.boundingBox.xs; z <- blockArray.boundingBox.zs) {
      ...
      if (comps.nonEmpty) {
        yUpFrom(seaLevel + 1).find(!blockAt(x, _, z).isSolid).foreach {
          yAboveGround =>
  ...
  def isValid =
    components.size >= 10 && components.last.widthStretch >= MaxStretch - 2
}

object RiverStructure {
  private def slide(start: Int, end: Int, width: Int): Range =
    start + ShorePadding to end - ShorePadding - width + 1 by width / 2
}
