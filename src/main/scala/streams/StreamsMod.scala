...
@Mod(
  ...
  modLanguage = "scala",
  useMetadata = true,
  dependencies = "after:terrafirmacraft"
)
object StreamsMod extends FarseekBaseMod {

  val configuration = None

  override val existingWorldWarning = Some(
    "Can cause unexpected behavior such as incomplete rivers."
  )
  ...
  @EventHandler override def handle(event: FMLLoadCompleteEvent) {
    ...
    if (event.getSide == CLIENT) {
      val customWaterIds = allBlocks
        .filter(_.getMaterial == Material.water)
        .map(getIdFromBlock)
        .toSeq
        .sorted
        .map("mc_Entity.x == " + _)
        .mkString(" || ")
      info(
        "Welcome to Farseek/Streams. Please visit http://www.minecraftforum.net/forums/mapping-and-modding/minecraft-mods/2346379-streams-real-flowing-rivers for information and updates."
      )
      info(
        "Shader configuration: if you wish to use shaders with custom water blocks including Streams river blocks, edit your shader's gbuffers_wqater.wsh file to replace the line:\n" +
