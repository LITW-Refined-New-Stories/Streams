...
public class MapGenCavesTFC extends MapGenBaseTFC {
     ...
     */
    protected void generateLargeCaveNode(long par1, int par3, int par4, Block[] par5, double par6, double par8,
            double par10) {
     ...
     */
    protected void generateCaveNode(long seed, int chunkX, int chunkZ, Block[] idArray, double i, double j, double k,
            float par12, float par13, float par14, int par15, int par16, double par17, double width) {
        ...
        for (boolean var28 = var25.nextInt(6) == 0; par15 < par16; ++par15) {
            ...
            if (var54 || var25.nextInt(4) != 0) {
                ...
                if (i >= worldX - 16.0D - var29 * 2.0D && k >= worldZ - 16.0D - var29 * 2.0D
                        && i <= worldX + 16.0D + var29 * 2.0D
                        && k <= worldZ + 16.0D + var29 * 2.0D) {
                        ...
                        for (zCoord = var56; !var58 && zCoord < var40; ++zCoord) {
                            for (int y = yCoord + 1; !var58 && y >= var57 - 1; --y) {
                                ...
                                if (y >= 0 && y < 256) {
                                    ...
                                    if (idArray[index + roofOffset] != null && idArray[index + roofOffset].getMaterial()
                                                                                                          .isLiquid())
                                        var58 = true;
                                    // delvr end
                                    if (y != var57 - 1 && xCoord != var55
                                            && xCoord != var36 - 1
                                            && zCoord != var56
                                            && zCoord != var40 - 1)
                                        y = var57;
                                ...
                                if (var59 * var59 + var46 * var46 < 1.0D) {
                                    for (int var50 = yCoord - 1; var50 >= var57; --var50) {
                                        ...
                                        if (var51 > -0.7D && var59 * var59 + var51 * var51 + var46 * var46 < 1.0D) {
                                            ...
                                            if (TFC_Core.isSoil(idArray[index])
                                                    || TFC_Core.isRawStone(idArray[index])) {
                                                if (TFC_Core.isSoilOrGravel(idArray[index + 1])) {
                                                    for (int upCount = 1; TFC_Core.isSoilOrGravel(
                                                            idArray[index + upCount]); upCount++) {
                                                        idArray[index + upCount] = Blocks.air;
                                                    }
                                                }

                                                if (var50 < 10 && TFC_Climate.getStability(
                                                        this.worldObj,
                                                        (int) worldX,
                                                        (int) worldZ) == 1) {
    ...
    protected void recursiveGenerate(World world, int par2, int par3, int par4, int par5, Block[] ids) {
        ...
        if (TFC_Climate.getCacheManager(world) != null) {
            DataLayer rockLayer1 = TFC_Climate.getCacheManager(world)
                                              .getRockLayerAt((int) xCoord, (int) zCoord, 0);
        ...
        if (this.rand.nextInt(caveChance) != 0) var7 = 0;
