...
package object tfc {
  ...
  def generate(
    ...
    if (
      between(y, y + 10).forall(!blockAt(x, _, z)(world).getMaterial.isLiquid)
    )
      generator.generate(world, random, x, y, z)
  }

  def generateStreams(
      generator: com.bioxx.tfc.WorldGen.TFCChunkProviderGenerate,
      world: World,
      xChunk: Int,
      zChunk: Int,
      blocks: Array[Block],
    ...
    surfaceWaterGenerator.onChunkGeneration(
  ...
  def tfcSurfaceBlockAt(
      wxz: XZ,
      world: World,
      sedimentOnly: Boolean = false
  ): BlockAndData = {
    ...
    val soilBlock =
      if (biome == BEACH || biome == OCEAN || biome == DEEP_OCEAN)
        getTypeForSand(soilData)
  ...
  def isFreshWater(block: Block): Boolean =
    block.isInstanceOf[BlockRiver] || TFC_Core.isFreshWater(block)

  def canReplace(
      fallingBlock: EntityFallingBlockTFC,
      world: World,
      x: Int,
      y: Int,
      z: Int
