...
public class TFCChunkProviderGenerate extends ChunkProviderGenerate {
    ...
    public Chunk provideChunk(int chunkX, int chunkZ) {
        ...
        if (TFC_Climate.getCacheManager(worldObj) != null) {
            rockLayer1 = TFC_Climate.getCacheManager(worldObj)
                                    .loadRockLayerGeneratorData(rockLayer1, chunkX * 16, chunkZ * 16, 16, 16, 0);
            rockLayer2 = TFC_Climate.getCacheManager(worldObj)
                                    .loadRockLayerGeneratorData(rockLayer2, chunkX * 16, chunkZ * 16, 16, 16, 1);
            rockLayer3 = TFC_Climate.getCacheManager(worldObj)
                                    .loadRockLayerGeneratorData(rockLayer3, chunkX * 16, chunkZ * 16, 16, 16, 2);
            evtLayer = TFC_Climate.getCacheManager(worldObj)
                                  .loadEVTLayerGeneratorData(evtLayer, chunkX * 16, chunkZ * 16, 16, 16);
            rainfallLayer = TFC_Climate.getCacheManager(worldObj)
                                       .loadRainfallLayerGeneratorData(rainfallLayer, chunkX * 16, chunkZ * 16, 16, 16);
            stabilityLayer = TFC_Climate.getCacheManager(worldObj)
                                        .loadStabilityLayerGeneratorData(
                                                ...
                                                16,
                                                16);
            drainageLayer = TFC_Climate.getCacheManager(worldObj)
                                       .loadDrainageLayerGeneratorData(drainageLayer, chunkX * 16, chunkZ * 16, 16, 16);
        }
        ...
        // delvr start
        if (this == ((WorldServer) worldObj).theChunkProviderServer.currentChunkProvider)
            RiverGenerator.surfaceWaterGenerator()
                          .generate(worldObj.provider, chunkX, chunkZ, idsBig, metaBig);
        else return new Chunk(this.worldObj, idsBig, metaBig, chunkX, chunkZ);
        // delvr end
    ...
    public void populate(IChunkProvider chunkProvider, int chunkX, int chunkZ) {
        ...
        if (this.worldObj.getBiomeGenForCoords(xCoord + 16, zCoord + 16) instanceof TFCBiome) // Fixes
                                                                                              // ClassCastException
        {
        ...
        TFC_Core.getCDM(worldObj)
                .setFishPop(chunkX, chunkZ, ChunkData.FISH_POP_MAX);

        int waterRand = 4;
        if (TFC_Climate.getStability(worldObj, xCoord, zCoord) == 1) waterRand = 6;
        ...
        MinecraftForge.EVENT_BUS.post(
                new PopulateChunkEvent.Post(chunkProvider, worldObj, rand, chunkX, chunkZ, var11));
    ...
    public static List<SpawnListEntry> getCreatureSpawnsByChunk(World world, TFCBiome biome, int x, int z) {
        ...
        float evt = 0.0f;
        if (TFC_Climate.getCacheManager(world) != null && TFC_Climate.getCacheManager(world)
                                                                     .getEVTLayerAt(x, z)
                != null)
            evt = TFC_Climate.getCacheManager(world)
                             .getEVTLayerAt(x, z).floatdata1;
        ...
        if (temp > 0 && temp < 21 && rain > 250) {
            spawnableCreatureList.add(
                    new SpawnListEntry(
                            EntityPigTFC.class,
                            2 + mountainousAreaModifier,
                            2 + mountainousAreaModifier,
                            3 + mountainousAreaModifier));
            spawnableCreatureList.add(new SpawnListEntry(EntityWolfTFC.class, 1, 1, 2 + mountainousAreaModifier));
            spawnableCreatureList.add(new SpawnListEntry(EntityBear.class, 1, 1, 1));
            spawnableCreatureList.add(
                    new SpawnListEntry(EntityDeer.class, 2 + mountainousAreaModifier, 1, 3 + mountainousAreaModifier));
        ...
        if (temp > -20 && temp <= 0) {
            ...
            if (rain > 250) {
                ...
                spawnableCreatureList.add(
                        new SpawnListEntry(EntityPheasantTFC.class, 1 + mountainousAreaModifier, 1, 2));
                spawnableCreatureList.add(new SpawnListEntry(EntitySheepTFC.class, 2, 2, 4));
            }
        ...
        if (temp >= 23 && temp < 44 && rain > 1500) {
            spawnableCreatureList.add(
                    new SpawnListEntry(
                            ...
                            2 + mountainousAreaModifier,
                            2 + mountainousAreaModifier,
                            4 + mountainousAreaModifier));
            spawnableCreatureList.add(
                    new SpawnListEntry(
                            EntityChickenTFC.class,
                            3 + mountainousAreaModifier,
                            1,
                            4 + mountainousAreaModifier));
        }
    ...
    public boolean canSnowAt(World world, int x, int y, int z) {
        ...
        if (var5 >= 0F) {
            ...
            if (y >= 0 && y < 256
                    && world.getSavedLightValue(EnumSkyBlock.Block, x, y, z) < 10
    ...
    public void generateTerrainHigh(int chunkX, int chunkZ, Block[] idsTop) {
        ...
        this.biomesForGeneration = this.worldObj.getWorldChunkManager()
                                                .getBiomesForGeneration(
                                                        ...
                                                        xSize + 5,
                                                        zSize + 5);
        this.noiseArray = this.initializeNoiseFieldHigh(
                this.noiseArray,
                chunkX * subDivXZ,
                0,
                chunkZ * subDivXZ,
                    ...
                    for (int var31 = 0; var31 < 8; ++var31) {
                        ...
                        for (int var42 = 0; var42 < 4; ++var42) {
                            ...
                            for (int var51 = 0; var51 < 4; ++var51) {
                                if ((var47 += var49) > 0.0D) idsTop[index += arrayYHeight] = Blocks.stone;
                                else if (y * 8 + var31 < seaLevel)
                                    idsTop[index += arrayYHeight] = TFCBlocks.saltWaterStationary;
                                else idsTop[index += arrayYHeight] = Blocks.air;
                            }
                            var34 += var38;
                            var36 += var40;
                        }
     ...
     * size.
     */
    private double[] initializeNoiseFieldHigh(double[] outArray, int xPos, int yPos, int zPos, int xSize, int ySize,
            int zSize) {
        if (outArray == null) outArray = new double[xSize * ySize * zSize];
        ...
        // this.noise5 = this.noiseGen5.generateNoiseOctaves(this.noise5, xPos, zPos, xSize, zSize, 1.121D, 1.121D,
        // 0.5D);
        ...
        this.noise3 = this.noiseGen3.generateNoiseOctaves(
                ...
                double2 / 160.0D,
                double1 / 80.0D);
        // this.noise3 = this.noiseGen3.generateNoiseOctaves(this.noise3, xPos, yPos, zPos, xSize, ySize, zSize, var44 /
        // 80.0D, 0.5, var44 / 80.0D);
            ...
            for (int z = 0; z < zSize; ++z) {
                ...
                for (int xR = -radius; xR <= radius; ++xR) {
                    for (int zR = -radius; zR <= radius; ++zR) {
                        ...
                        if (blendBiome.rootHeight > baseBiome.rootHeight) blendedHeight *= 0.5F;
                ...
                if (scaledNoise6Value < 0.0D) scaledNoise6Value = -scaledNoise6Value * 0.3D; // If negative, make
                                                                                             // positive and shrink by a
                                                                                             // third?

                scaledNoise6Value = scaledNoise6Value * 3.0D - 2.0D;

                if (scaledNoise6Value < 0.0D) // Only true when noise6[index2] is between -17,777 and 0,
                                              // scaledNoise6Value will be at maximum -2
                {
                    ...
                    if (scaledNoise6Value > 1.0D) scaledNoise6Value = 1.0D;
                ...
                for (int y = 0; y < ySize; ++y) {
                    ...
                    if (var38 < 0.0D) output = var34;
                    else if (var38 > 1.0D) output = var36;
                    else output = var34 + (var36 - var34) * var38;
    ...
    private void replaceBlocksForBiomeHigh(int chunkX, int chunkZ, Block[] idsTop, Random rand, Block[] idsBig,
            byte[] metaBig) {
        int seaLevel = 16;
        int worldHeight = 256;
        int indexOffset = 128;
        ...
        for (int xCoord = 0; xCoord < 16; ++xCoord) {
            for (int zCoord = 0; zCoord < 16; ++zCoord) {
                ...
                float rain = rainfallLayer[arrayIndexDL] == null ? DataLayer.RAIN_125.floatdata1
                ...
                if (TFC_Core.isBeachBiome(getBiome(xCoord - 1, zCoord).biomeID)
                        ...
                        || TFC_Core.isBeachBiome(getBiome(xCoord, zCoord - 1).biomeID)) {
                    if (!TFC_Core.isBeachBiome(getBiome(xCoord, zCoord).biomeID)) cliffMap[arrayIndex] = true;
                }
                for (int height = 127; height >= 0; --height) {
                    ...
                    if (TFC_Core.isBeachBiome(biome.biomeID) && height > seaLevel + h
                            && idsTop[index] == Blocks.stone) {
                        idsTop[index] = Blocks.air;
                        if (h == 0) h = (height - 16) / 4;
                    }
                    ...
                    if (idsBig[indexBig] == Blocks.stone) {
                        ...
                        if (chunkHeightMap[arrayIndex] == 0) chunkHeightMap[arrayIndex] = height + indexOffset;
                        ...
                        if (var13 == -1) {
                            ...
                            for (int counter = 1; counter < var12Temp / 3; counter++) {
                                if (arrayIndexx >= 0 && seaLevelOffsetMap[arrayIndex] - (3 * counter)
                                        > seaLevelOffsetMap[arrayIndexx]) {
                                    seaLevelOffsetMap[arrayIndex]--;
                                    var12--;
                                    height--;
                                    indexBig = (arrayIndex) * worldHeight + height + indexOffset;
                                    index = (arrayIndex) * 128 + height;
                                } else if (arrayIndexX >= 0 && seaLevelOffsetMap[arrayIndex] - (3 * counter)
                                        > seaLevelOffsetMap[arrayIndexX]) {
                                            seaLevelOffsetMap[arrayIndex]--;
                                            var12--;
                                            height--;
                                    ...
                                    if (arrayIndexz >= 0 && seaLevelOffsetMap[arrayIndex] - (3 * counter)
                            ...
                            if (var13 > 0) {
                                if (height >= seaLevel - 1 && index + 1 < idsTop.length
                                    ...
                                    for (int c = 1; c < dirtH && !TFC_Core.isMountainBiome(biome.biomeID)
                                            && biome != TFCBiome.HIGH_HILLS
                                            && biome != TFCBiome.HIGH_HILLS_EDGE
                                            && !cliffMap[arrayIndex]; c++) {
                        ...
                        if (height > seaLevel - 2 && height < seaLevel
                            ...
                            && biome != TFCBiome.DEEP_OCEAN
                            && biome != TFCBiome.BEACH
    ...
    public void convertStone(int height, int indexArray, int indexBig, Block[] idsBig, byte[] metaBig, DataLayer rock1,
            DataLayer rock2, DataLayer rock3) {
        if (idsBig[indexBig] != null && idsBig[indexBig] != Blocks.stone) return;
        if (height <= TFCOptions.rockLayer3Height + seaLevelOffsetMap[indexArray]) {
            ...
            metaBig[indexBig] = (byte) rock3.data2;
                ...
                && height > 55 + seaLevelOffsetMap[indexArray]
                && rock2 != null) {
