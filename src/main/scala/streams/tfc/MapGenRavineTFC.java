...
public class MapGenRavineTFC extends MapGenBaseTFC {
    ...
    protected void generateRavine(long seed, int chunkX, int chunkZ, Block[] blockArray, double xCoord, double yCoord,
            double zCoord, float par12, float par13, float par14, int par15, int par16, double yScale) {
        ...
        for (int var28 = 0; var28 < 256; ++var28) {
            if (var28 == 0 || var19.nextInt(3) == 0) var27 = 1.0F + var19.nextFloat() * var19.nextFloat() * 1.0F;
        ...
        for (; par15 < par16; ++par15) {
            ...
            if (var54 || var19.nextInt(4) != 0) {
                ...
                if (xCoord >= chunkMidX - 16.0D - var53 * 2.0D && zCoord >= chunkMidZ - 16.0D - var53 * 2.0D
                        && xCoord <= chunkMidX + 16.0D + var53 * 2.0D
                        && zCoord <= chunkMidZ + 16.0D + var53 * 2.0D) {
                        ...
                        for (int z = zMin; !isBlocked && z < zMax; ++z) {
                            for (int y = maxY + 1; !isBlocked && y >= minY - 1; --y) {
                                ...
                                if (y >= 0 && y < 256) {
                                    ...
                                    if (blockArray[index + roofOffset] != null
                                            && blockArray[index + roofOffset].getMaterial()
                                                                             .isLiquid())
                                        isBlocked = true;
                                    // delvr end
                            ...
                            for (index = zMin; index < zMax; ++index) {
                                ...
                                if (var59 * var59 + var45 * var45 < 1.0D) {
                                    for (int var49 = maxY - 1; var49 >= minY; --var49) {
                                        ...
                                        if ((var59 * var59 + var45 * var45) * this.multipliers[var49]
                                            ...
                                            if (TFC_Core.isGround(blockArray[index2])) {
                                                if (TFC_Core.isSoilOrGravel(blockArray[index2 + 1])) {
                                                    for (int upCount = 1; TFC_Core.isSoilOrGravel(
                                                            blockArray[index2 + upCount]); upCount++) {
                                                        blockArray[index2 + upCount] = Blocks.air;
                                                    }
                                                }

                                                if (var49 < 10) blockArray[index2] = Blocks.lava;
                                                else blockArray[index2] = Blocks.air;
                                            }
                                        }
