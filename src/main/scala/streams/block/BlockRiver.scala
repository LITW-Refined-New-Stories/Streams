...
class BlockRiver(liquid: MaterialLiquid, val dx: Int, val dz: Int)
    extends BlockLiquid(liquid)
    with IFluidBlock
    with FixedFlowBlock {
  ...
  override def onBlockAdded(w: World, x: Int, y: Int, z: Int) {
    if (populating)
      ...
      liquid.flowingBlock.onBlockAdded(
        w,
        x,
        y,
        z
      ) // Checks for hardening & schedules update
  }
  ...
  override def onNeighborBlockChange(
    ...
    if (formerNeighbor.isSolidOrLiquid) {
      ...
      if (populating)
        ...
        liquid.flowingBlock.onNeighborBlockChange(
          w,
          x,
          y,
          z,
          formerNeighbor
        ) // Checks for hardening
        if (
          blockAt(x, y, z) == this && (!blockBelow(
            x,
            y,
            z
          ).isSolidOrLiquid || neighbors(x, y, z)
            .map(blockAt)
            .exists(!_.isSolidOrLiquid))
        )
  ...
  private def shoreUp(xyz: XYZ)(implicit w: World) {
    stabilize(xyz.below)
    allNeighbors(xyz.x, xyz.y, xyz.z).foreach(
      stabilize
    ) // All 8 neighbors, to prevent diagonally placed mid-air TFC stone blocks from dropping as items
  }

  private def stabilize(xyz: XYZ)(implicit w: World) {
    val block = blockAt(xyz)
    if (
      !block.isSolidOrLiquid || (block.isGranular && !blockBelow(xyz).isSolid)
    )
      setBlockAndDataAt(
        xyz,
        rockBlockFor(xyz.x, xyz.y, xyz.z),
  ...
  override def breakBlock(
    ...
    if (
      !w.scheduledUpdatesAreImmediate && newBlock.isSolid && newBlock.getMaterial != Material.rock
    ) {
      if (
        populating || entityPresent(xyz, classOf[EntityFallingBlock]) ||
        (tfcLoaded && entityPresent(
          xyz,
          classOf[EntityFallingBlockTFC]
        )) || blocksFallInstantlyAt(xyz)
      )
  ...
  override def getFlowVector(w: IBlockAccess, x: Int, y: Int, z: Int) = { // Normalized in World.handleMaterialAcceleration()
    ...
    if (dataAt(x, y, z) == 0)
      ...
      val fallingNeighborDirections = CompassDirections.filter(d =>
        blockAbove(x + d.x, y, z + d.z).getMaterial == liquid
      )
      if (fallingNeighborDirections.isEmpty)
        ...
        val dMax =
          abs_max(combinedDirection.x.toDouble, combinedDirection.z.toDouble)
        createVectorHelper(
          -round(combinedDirection.x / dMax * 2).toInt,
          0,
          -round(combinedDirection.z / dMax * 2).toInt
        )
      }
    }
  }
  ...
  private def tryToFlowInto(xyz: XYZ, decay: Int, isBelow: Boolean = false)(
      implicit w: World
  ) {
    if (!blockAt(xyz).isSolidOrLiquid) {
      val riverNeighbors = xyz.neighbors.flatMap { case (nx, ny, nz) =>
        blockAt(nx, xyz.y, nz) match {
          case riverBlock: FixedFlowBlock =>
            Some((riverBlock.dx, riverBlock.dz, dataAt(nx, ny, nz)))
          case _ => None
        }
      }
  ...
  @SideOnly(CLIENT)
  override def colorMultiplier(w: IBlockAccess, x: Int, y: Int, z: Int) =
    liquid.stillBlock.colorMultiplier(w, x, y, z) // for TFC
  ...
  override def randomDisplayTick(
    ...
    if (
      liquid == Material.water && dataAt(x, y, z)(w) == 0 && oneChanceOutOf(64)(
        random
      )
    )
      w.playSound(
  ...
  def getFilledPercentage(w: World, x: Int, y: Int, z: Int) = {
    implicit val world = w
    if (blockAbove(x, y, z).isLiquid) 1f
    else {
      val data = dataAt(x, y, z)
      val decay =
        if (
          data != 0 && allNeighbors(x, y, z).exists(
            blockAbove(_).getMaterial == liquid
          )
        ) 0f
        else data.toFloat
      (8f - decay) / 9f
