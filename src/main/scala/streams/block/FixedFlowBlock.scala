...
object FixedFlowBlock {
  ...
  private val FixedFlowBlocks: Map[(Material, Int, Int), FixedFlowBlock] =
    (for (
      material <- Seq(Material.water, Material.lava).map(
        _.asInstanceOf[MaterialLiquid]
      );
      dx <- -2 to 2; dz <- -2 to 2;
  ...
  def apply(material: Material): Seq[FixedFlowBlock] =
    FixedFlowBlocks.filter(_._1._1 == material).values.toSeq

  def apply(material: Material, flow: XZ): FixedFlowBlock =
    apply(material, flow.x, flow.z)

  def apply(material: Material, dx: Int, dz: Int): FixedFlowBlock =
    FixedFlowBlocks((material, dx, dz))
  ...
  def interpolate(flows: (Int, Int, Int)*): (Double, Double, Int) = {
    val (dxSum, dzSum, decaySum) =
      flows.reduce((acc, v) => (acc.x + v.x, acc.y + v.y, acc.z + v.z))
    val divisor = flows.size.toDouble
